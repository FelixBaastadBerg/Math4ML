...
EVAL_DIR = Path("./MLP_ECE/Evaluation") / METHOD
...
def plot_reliability_diagram(
    ...
    if show_confidence:
        conf = df_bins["confidence"].values
        ax.scatter(centers[mask], conf[mask], s=24, label="Mean confidence")
...
for n, paths in pairs.items():
    ...
    if proba is not None:
        # Uniform-binned ECE (standard)
        df_bins_u, ece_uniform, mce_uniform = calibration_table(yte, proba, n_bins=15, strategy="uniform")
        df_bins_u.to_csv(out_dir / "calibration_bins_uniform.csv", index=False)
        plot_reliability_diagram(
            df_bins_u,
            title=f"Kryptonite-{n} : Reliability (Uniform bins, ECE={ece_uniform:.3f})",
            save_path=out_dir / "reliability_uniform.png",
        ...
        plot_reliability_diagram(
            df_bins_q,
            title=f"Kryptonite-{n} : Reliability (Adaptive bins, ECE={ece_adapt:.3f})",
            save_path=out_dir / "reliability_adaptive.png",
            show_confidence=True
        )
    ...
    summary_rows.append({
        ...
        "mce_uniform": mce_uniform,
        "ece_adaptive": ece_adapt,
        "mce_adaptive": mce_adapt,
    })
...
TEST_SUMMARY = (Path("./MLP_ECE/Evaluation") / METHOD / "summary_test_metrics.csv")
SAVE_DIR     = (Path("./MLP_ECE/Evaluation") / METHOD)
